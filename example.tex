...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\path (lane1-left) +(1,0) node {\carr};
		
		\tikzstyle{every pin}=[message]
		
		\begin{pgfonlayer}{cars}
		\path (lane1-left) +(3,0) node[pin=below:\textbf{Warning:} emergency breaking!] (car1) {\carr};
		\path (lane2-left) +(4,0) node[pin=above:\textbf{Warning:} emergency breaking!] (car2) {\carr};
		\end{pgfonlayer}
